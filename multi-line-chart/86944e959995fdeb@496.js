...
function getTime(laptime){
  const time = laptime.split(".");
  const minutes = parseFloat(time[0]);
  const seconds = parseFloat(time[1]);
  const tenths = parseFloat(time[2]);
  return minutes*60 + seconds + tenths/ (10 ** tenths.toString().length );
}
...
function LineChart(data, {
  ...
  yFormat,
  xLabel, // a format specifier string for the y-axis
  ...
  const xAxis = d3.axisBottom(xScale).ticks(width / 50 + 30).tickSizeOuter(0);
  const yAxis = d3.axisLeft(yScale).ticks(height / 60 + 30, yFormat);
  ...
  const path = svg.append("g")
    ...
    .join("path")
      ...
      .style("stroke-dasharray", typeof dashed === "function" ? ([z]) => dashed(z) : null)
      .attr("d", ([, I]) => line(I));
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["bls-metro-unemployment.csv", {url: new URL(`../datasets/races/${top.raceName}.csv`, import.meta.url), mimeType: "text/csv", toString}]
  ]);
